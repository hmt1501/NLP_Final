...
REM Windows batch script to run LED parameter tuning (FIXED)
...
for %%d in (0.05 0.1 0.2) do (
    for %%l in (5e-4 1e-5 5e-5 1e-6 5e-6) do (
        ...
        if not exist "!EXPERIMENT_PATH!" (
            ...
            D:\miniconda\envs\all\python.exe summarization\run_summarization.py ^
                --model_name_or_path allenai/led-large-16384 ^
                --do_train --do_eval --do_predict ^
                --train_file "%DATA_PATH%\train_4000_600_chars.json" ^
                --validation_file "%DATA_PATH%\valid_4000_600_chars.json" ^
                --test_file "%DATA_PATH%\test_4000_600_chars_last_100.json" ^
