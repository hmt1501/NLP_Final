...
class ModelArguments:
    ...
    use_lora: bool = field(
        default=False,
        metadata={"help": "Whether to use lora for parameter efficient finetuning."},
    )
    dropout: float = field(
        default=0.1,
...
def main():
    # Fix Windows encoding issue FIRST
    import sys
    import os
    ...
    # Disable wandb to avoid logging issues
    ...
    # Enable wandb logging - DISABLED due to encoding issues
    ...
    # import pickle
    # with open('../args/args_sum.pkl', 'wb') as f:
    #     args = {
    ...
    #     if model == 'flan-t5-xxl' or model == 'philschmid/flan-t5-xxl-sharded-fp16':
    #         peft_config = LoraConfig(
    #             task_type=TaskType.SEQ_2_SEQ_LM, inference_mode=False, r=16, lora_alpha=32, target_modules=["q", "v"], lora_dropout=0.05, bias="none"
    #         )
    ...
    # add special tokens and resize token embeddings
    # special_tokens_dict = {'additional_special_tokens': ['[CPT1]', '[CPT2]', '[CPT3]', '[CPTE]']}
    ...
    def preprocess_function(examples):
        ...
        # For fine-tuning LED-large-16384, we need to apply a fix:
        # https://github.com/huggingface/transformers/issues/18190#issuecomment-1273482690
        if model_args.model_name_or_path == 'allenai/led-large-16384':
            ...
            print("Fixing labels for LED-large-16384")
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            # Replace -100 in the preds as we can't decode them.
                test_results_predictions = np.where(test_results.predictions != -100, test_results.predictions, tokenizer.pad_token_id)
                test_preds = tokenizer.batch_decode(
                    test_results_predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True
                )
                ...
                with open(os.path.join(training_args.output_dir, "test_generations.pkl"), "wb") as f:
